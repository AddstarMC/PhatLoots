...
public class WorldGuardRegionHook implements RegionHook {
    ...
    public List<String> getRegionNames(Location loc) {
        ...
        RegionManager container = WorldGuard.getInstance().getPlatform().getRegionContainer().get(BukkitAdapter.adapt(loc.getWorld()));
        if (container != null) {
            ApplicableRegionSet applicableRegionSet = container.getApplicableRegions(BukkitAdapter.asBlockVector(loc));
